...
```
🐳  → docker service create --name perf-test-a --network perf-test akyumurkov/netshoot iperf -s -p 9999
7dkcckjs0g7b4eddv8e5ez9nv


🐳  → docker service create --name perf-test-b --network perf-test akyumurkov/netshoot iperf -c perf-test-a -p 9999
...
2yb6fxls5ezf  perf-test-b  1/1       akyumurkov/netshoot  iperf -c perf-test-a -p 9999
7dkcckjs0g7b  perf-test-a  1/1       akyumurkov/netshoot  iperf -s -p 9999



🐳  → docker ps
CONTAINER ID        IMAGE                      COMMAND                  CREATED             STATUS              PORTS               NAMES
ce4ff40a5456        akyumurkov/netshoot:latest   "iperf -s -p 9999"       31 seconds ago      Up 30 seconds                           perf-test-a.1.bil2mo8inj3r9nyrss1g15qav
...
🐳  → docker run -it --net container:perf-test-a.1.0qlf1kaka0cq38gojf7wcatoa  akyumurkov/netshoot 
...
```
🐳  → docker run -it --privileged akyumurkov/netshoot nmap -p 12376-12390 -dd 172.31.24.25
...
```
 → docker ps
CONTAINER ID        IMAGE                      COMMAND                  CREATED             STATUS              PORTS               NAMES
ce4ff40a5456        akyumurkov/netshoot:latest   "iperf -s -p 9999"       5 minutes ago       Up 5 minutes                            perf-test-a.1.bil2mo8inj3r9nyrss1g15qav

🐳  → docker run -it --net container:perf-test-a.1.bil2mo8inj3r9nyrss1g15qav akyumurkov/netshoot iftop -i eth0
...
```
🐳  → docker run -it --net container:perf-test-a.1.bil2mo8inj3r9nyrss1g15qav akyumurkov/netshoot drill -V 5 perf-test-b
...
🐳  → docker service create --name service-a --network my-ovl -p 8080:8080 akyumurkov/netshoot nc -l 8080
bnj517hh4ylpf7ewawsp9unrc

🐳  → docker service create --name service-b --network my-ovl akyumurkov/netshoot nc -vz service-a 8080
...
🐳  →  docker run -d --rm --net br --name c1 akyumurkov/netshoot netgen c2 5000
8c51eb2100c35d14244dcecb80839c780999159985415a684258c7154ec6bd42

🐳  →  docker run -it --rm --net br --name c2 akyumurkov/netshoot netgen c1 5000
...
# Launching netshoot in privileged mode
 🐳  → docker run -it --rm -v /var/run/docker/netns:/var/run/docker/netns --privileged=true akyumurkov/netshoot
...
`/ # docker run -it --rm -v /var/run/docker.sock:/var/run/docker.sock akyumurkov/netshoot ctop`
...
/ # docker run --rm --cap-add=NET_ADMIN --cap-add=NET_RAW -it akyumurkov/netshoot termshark -i eth0 icmp
```

```
# Launching netshoot with NET_ADMIN and CAP_NET_RAW capabilities Reading packets from ipv4frags.pcap

/ # docker run --rm --cap-add=NET_ADMIN --cap-add=NET_RAW -v /tmp/ipv4frags.pcap:/tmp/ipv4frags.pcap -it akyumurkov/netshoot termshark -r /tmp/ipv4frags.pcap
```
